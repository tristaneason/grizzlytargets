...
<?php
	global $MWQDC_LB;
	global $wpdb;
	$page_url = 'admin.php?page=mw-qbo-desktop-push&tab=product';
	
	$MWQDC_LB->set_per_page_from_url();
	$items_per_page = $MWQDC_LB->get_item_per_page();

	$MWQDC_LB->set_and_get('product_push_search');
	$product_push_search = $MWQDC_LB->get_session_val('product_push_search');
	...
	$total_records = $MWQDC_LB->count_woocommerce_product_list($product_push_search,false,$product_stock_srch,$product_cat_search,$product_um_srch);
	...
	$wc_product_list = $MWQDC_LB->get_woocommerce_product_list($product_push_search," $offset , $items_per_page",false,$product_stock_srch,$product_cat_search,$product_um_srch);
...
<div class="container mwqbd-cnt">
	<div class="page_title"><h4><?php _e( 'Product Push', 'mw_wc_qbo_desk' );?></h4></div>
	<div class="card">
						        ...
						        <div class="panel panel-primary">
									 ...
									 <div class="row">
										<div class="input-field col s12 m12 14">
											<button id="push_selected_product_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green"><?php echo __('Push Selected Products','mw_wc_qbo_desk')?></button>
											<button style="display:none;" id="push_all_product_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green hide"><?php echo __('Push All Products','mw_wc_qbo_desk')?></button>
											<button style="display:none;" disabled="disabled" id="push_all_unsynced_product_btn" class="waves-effect waves-light btn save-btn mw-qbo-sync-green hide"><?php echo __('Push Un-synced Products','mw_wc_qbo_desk')?></button>
										</div>
									</div>
									 <br />
									...
									<div class="table-m">
										<div class="myworks-wc-qbd-sync-table-responsive">
											<table class="table tablesorter" id="mwqs_product_push_table_desk">
												<thead>
													<tr>
														<th width="2%">
														<input type="checkbox" onclick="javascript:mw_qbo_sync_check_all_desk(this,'product_push_')">
														</th>
														<th width="5%">ID</th>
												...
												if($show_sync_status){
													...
													if($p_val['quickbook_product_id']){
														...
														//
														$dp_id = $MWQDC_LB->get_data_pair_id('Product',$quickbook_product_id);
													}
												}
...
<?php $sync_window_url = $MWQDC_LB->get_sync_window_url();?>
 <script type="text/javascript">
	function search_item(){		
		...
		if(product_push_search!='' || product_stock_srch!='' || product_cat_search!='' || product_um_srch!=''){			
			window.location = '<?php echo $page_url;?>&product_push_search='+product_push_search+'&product_stock_srch='+product_stock_srch+'&product_cat_search='+product_cat_search+'&product_um_srch='+product_um_srch;
		}else{
			alert('<?php echo __('Please enter search keyword or select stock status or category or mapped/unmapped.','mw_wc_qbo_desk')?>');
		}
	}

	function reset_item(){		
		window.location = '<?php echo $page_url;?>&product_push_search=&product_stock_srch=&product_cat_search=&product_um_srch=';
	}
	...
	jQuery(document).ready(function($) {
		...
		$('#push_selected_product_btn').click(function(){
			...
			popUpWindowDesk('<?php echo $sync_window_url;?>&sync_type=push&item_ids='+item_ids+'&item_type='+item_type,'mw_qs_product_push_desk',0,0,650,350);
			return false;
		});
		
		$('#push_all_product_btn').click(function(){
			popUpWindowDesk('<?php echo $sync_window_url;?>&sync_type=push&sync_all=1&item_type='+item_type,'mw_qs_product_push_desk',0,0,650,350);
			return false;
		});
		
		$('#push_all_unsynced_product_btn').click(function(){
			popUpWindowDesk('<?php echo $sync_window_url;?>&sync_type=push&sync_unsynced=1&item_type='+item_type,'mw_qs_product_push_desk',0,0,650,350);
			return false;
		});
	});
 </script>
 <?php echo $MWQDC_LB->get_tablesorter_js('#mwqs_product_push_table_desk');?>
