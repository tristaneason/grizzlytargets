...
//		'button_url' => $free_plugin_base_url . 'wp-sheet-editor-autofill',
			...
			if (!empty(VGSE()->options['be_allowed_user_roles'])) {
				if (!is_user_logged_in()) {
					...
					if (!in_array($first_role, VGSE()->options['be_allowed_user_roles'])) {
						VGSE()->options['be_rest_api_only'] = true;
					}
				}
			}

			if (!empty(VGSE()->options['be_rest_api_only'])) {
				...
				add_filter('redux/options/' . VGSE()->options_key . '/args', array($this, 'move_settings_to_general_options'));
			}
		...
		function clear_cache_after_term_edited($data, $term_id, $taxonomy, $args) {
			...
			if ($must_cleanup) {
				delete_transient('wpse_terms_' . $taxonomy);
			}
			return $data;
		}
		...
		function move_settings_to_general_options($args) {
			$args['page_parent'] = 'options-general.php';
			$args['menu_title'] = __('WP Sheet Editor', VGSE()->textname);
			return $args;
		}
		...
		function catch_missing_reduxframework_error() {
			...
			if (!empty($_GET['page']) && $_GET['page'] === VGSE()->options_key && !class_exists('ReduxFramework')) {
				echo '<p>' . __(sprintf('WP Sheet Editor: Please install the Redux Framework plugin. <a href="%s" target="_blank" class="button">Click here</a>.<br/>It´s required for the settings page, you can remove redux framework after changing the settings.', $this->get_plugin_install_url('redux-framework')), VGSE()->textname) . '</p>';
				die();
			}
		}
...
if (!function_exists('vgse_force_editor_in_english')) {
	...
	function vgse_force_editor_in_english() {
		if (isset($_GET['page']) && strpos($_GET['page'], 'vgse-bulk-edit-') !== false && is_admin() && is_rtl()) {
