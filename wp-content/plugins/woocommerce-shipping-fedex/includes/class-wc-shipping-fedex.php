...
class WC_Shipping_Fedex extends WC_Shipping_Method {
	...
	private function set_settings() {
		...
		$this->direct_distribution        = 'yes' === $this->get_option( 'direct_distribution' );
	...
	public function get_fedex_packages( $package ) {
		switch ( $this->packing_method ) {
			case 'box_packing' :
				return $this->try_box_shipping( $package );
				break;
			case 'per_item' :
			default :
	...
	/**
	 * Try to pack into boxes with weights and dimensions.
	 * If One Rates are enabled we will initially try to pack with One Rate limitations.
	 * If one of the items does not fit we will try to repack again with normal limitations.
	 *
	 * @param mixed $package Package to ship.
	 * @return array
	 */
	private function try_box_shipping( $package ) {
		if ( $this->fedex_one_rate && 'US' === $package['destination']['country'] && 'US' === $this->origin_country ) {
			$boxes = $this->box_shipping( $package );

			// Confirm all items are packed into supported boxes.
			$packed = true;
			foreach ( $boxes as $box ) {
				if ( ! in_array( $box['package_id'], $this->fedex_one_rate_package_ids, true ) ) {
					$packed = false;
					break;
				}
			}
			...
			if ( $packed ) {
				// No unpacked items, so return list of One Rate packed items.
				return $boxes;
			}

			// Continue with One Rate disabled.
			$this->fedex_one_rate = false;
		}
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( $fedex_requests ) {
			...
			if ( ! empty( $this->custom_services['FEDEX_GROUND']['enabled'] ) && ( ! $this->is_shipping_internationally() || $this->need_direct_distribution() ) ) {
	...
	public function add_found_rates() {
		if ( $this->found_rates ) {
			// Remove ground rates if shipping internationally and direct distribution is disabled.
			if ( $this->is_shipping_internationally() && ! $this->need_direct_distribution() ) {
				unset( $this->found_rates[ 'fedex:' . $this->instance_id . ':FEDEX_GROUND' ] );
			}
	...
	public function need_direct_distribution() {
		if ( $this->direct_distribution ) {
			if ( 'US' === $this->origin_country && 'CA' === $this->package['destination']['country'] ) {
				return true;
			}

			if ( 'CA' === $this->origin_country && 'US' === $this->package['destination']['country'] ) {
				return true;
			}
		}
