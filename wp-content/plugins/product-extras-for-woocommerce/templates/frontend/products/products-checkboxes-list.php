...
<?php if( $item['child_products'] ) {
	...
	foreach( $item['child_products'] as $child_product_id ) {
	  ...
	  $checkbox = sprintf(
	    '<div class="%s"><label for="%s"><input data-option-cost="%s" data-field-label="%s" type="checkbox" name="%s[]" id="%s" class="pewc-checkbox-form-field" value="%s" %s %s><span class="pewc-theme-element"></span></label><div class="pewc-checkboxes-list-desc-wrapper">%s<div class="pewc-checkboxes-list-desc">%s</div></div></div>',
