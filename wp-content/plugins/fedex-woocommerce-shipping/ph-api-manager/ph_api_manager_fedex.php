...
if ( ! class_exists( 'PH_FedEx_API_Manager' ) ) {
	class PH_FedEx_API_Manager {
		...
		public function inactive_notice() { ?>
            ...
            <div class="notice notice-error">
                <p><?php printf( __( 'The <strong>%s</strong> API Key has not been activated, so the %s will not get future updates! %sClick here%s to activate <strong>%s</strong> API Key.', $this->text_domain ), esc_attr( $this->software_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'options-general.php?page=' . $this->wc_am_activation_tab_key ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
            </div>
			<?php
		}
		...
		public function wc_am_api_key_status() {
			...
			$license_status_check = ( ! empty( $license_status[ 'status_check' ] ) && $license_status[ 'status_check' ] == 'active' ) ? esc_html__( 'Activated', $this->text_domain ) : esc_html__( 'Deactivated', $this->text_domain );
		...
		public function get_api_key_status( $live = false ) {
			 ...
			 */
			if ( $live ) {
				return $this->wc_am_api_key_status() == 'Activated' ? true : false;
			}
		...
		public function wc_am_product_id_field() {
			...
			if ( ! empty( $product_id ) ) {
				...
				echo "<input id='api_key' name='" . esc_attr( $this->wc_am_product_id ) . "' size='25' type='text' value='' />";
			}
		}
			...
			if ( $_REQUEST[ 'option_page' ] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( isset( $activate_results[ 'data' ][ 'error_code' ] ) && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						...
						if( isset( $this->data[ $this->wc_am_activated_key ] ) )
						{
							update_option( $this->data[ $this->wc_am_activated_key ], 'Deactivated' );
						}
					}
		...
		public function update_check( $transient ) {
			...
			$args = array(
				...
				'product_id'  => $this->product_id,
				'api_key'     => isset($this->data[ $this->wc_am_api_key_key ]) ? $this->data[ $this->wc_am_api_key_key ] : '',
				'instance'    => $this->wc_am_instance_id,
			);
