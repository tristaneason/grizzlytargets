...
if ($loading_overlay !== 'no' && ($loading_overlay === 'yes' || ($loading_overlay !== 'yes' && $porto_settings['show-loading-overlay']))) {
...
<?php
	...
	if (porto_show_archive_filter()) {
		if ($porto_layout === 'fullwidth') {
			$porto_layout = 'left-sidebar';
		}
		if ($porto_layout === 'widewidth') {
...
?>
	...
	<div class="page-wrapper<?php echo !$header_is_side ? '' : ' side-nav', isset($porto_settings['header-side-position']) && $porto_settings['header-side-position'] ? ' side-nav-right' : ''; ?>">
		...
		if ($porto_settings['sticky-header-effect'] === 'reveal') {
			$header_wrapper_class_escaped .= ' header-reveal';
		}
		if (!($header_is_side && $wrapper === 'boxed') && ($porto_banner_pos === 'below_header' || $porto_banner_pos === 'fixed' || porto_get_meta_value('header_view') === 'fixed') || $porto_settings['header-view'] === 'fixed') {
		...
		<div id="main" class="<?php echo esc_attr(implode(' ', $main_class)); ?>"><!-- main -->
			...
			if ($wrapper === 'boxed' || $porto_layout === 'fullwidth' || $porto_layout === 'left-sidebar' || $porto_layout === 'right-sidebar' || 'both-sidebar' == $porto_layout) {
			...
			<div class="row main-content-wrap<?php echo isset($porto_shop_filter_layout) && $porto_shop_filter_layout === 'horizontal' ? ' porto-products-filter-body' : ''; ?>">
