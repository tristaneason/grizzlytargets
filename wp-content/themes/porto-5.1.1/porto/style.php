...
.container-fluid .vc_row:not(.porto-inner-container) { padding-left: <?php echo (int) $b['grid-gutter-width'] / 2; ?>px; padding-right: <?php echo (int) $b['grid-gutter-width'] / 2; ?>px; }
...
@media (max-width: 991px) {
	...
	.container-fluid .porto-inner-container .container { padding-left: <?php echo (int) $b['grid-gutter-width']; ?>px; padding-right: <?php echo (int) $b['grid-gutter-width']; ?>px; }
}
...
ul.products,
.slider-wrapper,
.vc_section { margin-left: -<?php echo (int) $b['grid-gutter-width'] / 2; ?>px; margin-right: -<?php echo (int) $b['grid-gutter-width'] / 2; ?>px; }
...
<?php if ( 'minicart-arrow-alt' == $minicart_type ) : ?>
	...
	#header.sticky-header #mini-cart .minicart-icon { font-size: 25px; }
	...
	@media (max-width: 991px) {
	<?php if ( $header_type ) : ?>
		#header:not(.sticky-header) #mini-cart { margin-<?php echo porto_filter_output( $left ); ?>: .5rem }
	<?php endif; ?>
...
<?php elseif ( 'simple' == $minicart_type ) : ?>
	...
	#mini-cart .cart-items { background: #ed5348; width: 15px; height: 15px; line-height: 15px; <?php echo porto_filter_output( $right ); ?>: -11px; top: -4px; font-size: 9px; box-shadow: -1px 1px 2px 0 rgba(0, 0, 0, 0.3); }
...
<?php if ( isset( $b['submenu-arrow'] ) && $b['submenu-arrow'] ) : ?>
	...
	/* menu effect down */
	.mega-menu > li.has-sub:before,
	.mega-menu > li.has-sub:after { bottom: 4px; transition: bottom .2s ease-out; }
	.mega-menu > li.has-sub:before { bottom: 5px; }
	.mega-menu > li.has-sub:hover:before { bottom: 0; }
	.mega-menu > li.has-sub:hover:after { bottom: -1px; }
...
<?php elseif ( 8 == $header_type ) : ?>
	...
	#mini-cart .cart-popup:before { <?php echo porto_filter_output( $right ); ?>: 28.7px; }
	#mini-cart .cart-popup:after { <?php echo porto_filter_output( $right ); ?>: 28px; }
...
<?php elseif ( 12 == $header_type ) : ?>
	...
	#header .share-links a:not(:hover) { background: none; <?php if ( $b['header-link-color']['regular'] ) { echo 'color: ' . esc_html( $b['header-link-color']['regular'] ); } ?> }
	@media (max-width: 991px) {
...
<?php elseif ( 'side' == $header_type ) : ?>
	...
	@media (min-width: 992px) {
		...
		.header-wrapper #header { min-height: 100vh; padding: 10px 15px 160px; position: relative; }
	...
	<?php if ( class_exists( 'Woocommerce' ) ) : ?>
		...
		#mini-cart .cart-head:after { content: '\e81c'; font-family: 'porto'; font-size: 15px; margin-<?php echo porto_filter_output( $left ); ?>: 4px; vertical-align: top; }
...
/* member */
.member-row .member { padding: 0 <?php echo esc_html( $b['grid-gutter-width'] / 2 ); ?>px 1px; margin-bottom: <?php echo (int) $b['grid-gutter-width']; ?>px; }
...
<?php if ( $dark ) : ?>
	...
	.slider-title .line { background-image:linear-gradient(to <?php echo porto_filter_output( $right ); ?>, rgba(255, 255, 255, 0.15), rgba(255, 255, 255, 0.15) 70%, rgba(255, 255, 255, 0) 100%); }
	@media (max-width: 767px) {
...
<?php if ( class_exists( 'Woocommerce' ) ) : ?>
	<?php if ( $dark ) : ?>
		...
		ul.product_list_widget li .ajax-loading { background-color: <?php echo esc_html( $color_dark_3 ); ?>; }
		...
		.woocommerce-tabs .tab-content { border-color: <?php echo esc_html( $color_dark_4 ); ?>; background: none; }
		.woocommerce-ordering select,
		.woocommerce-viewing select,
		.shop-loop-after,
		.woocommerce-widget-layered-nav-list a:not(.filter-color),
		.filter-item-list .filter-item { border-color: <?php echo esc_html( $color_dark_3 ); ?>; }
		.single-product .entry-summary .add_to_wishlist:before,
		.single-product .entry-summary .yith-wcwl-wishlistexistsbrowse a:before,
		.single-product .entry-summary .yith-wcwl-wishlistaddedbrowse a:before,
		.single_variation_wrap, .single-product form.cart:not(.variations_form) { border-color: <?php echo esc_html( $color_dark_4 ); ?> !important; }
		.quantity .qty,
		.gridlist-toggle > a,
		...
		.woocommerce-tabs h2.resp-accordion { color: <?php echo esc_html( $b['light-color'] ); ?>; }
		...
		.single-product .entry-summary .yith-wcwl-wishlistaddedbrowse a:hover:before { background: <?php echo esc_html( $color_dark_3 ); ?> !important; }
		#mini-cart .total-count,
		#mini-cart .cart-popup:after { border-bottom-color: <?php echo esc_html( $color_dark_4 ); ?>; }
...
<?php if ( class_exists( 'Woocommerce' ) ) : ?>
	...
	<?php if ( isset( $b['woo-show-product-border'] ) && $b['woo-show-product-border'] ) : ?>
		.product-image { border: 1px solid <?php echo esc_html( $dark ? $color_dark_3 : '#e7e7e7' ); ?>; /*width: 99.9999%;*/ }
	<?php endif; ?>
	<?php if ( ! $b['thumb-padding'] ) : ?>
		.product-images .product-image-slider.owl-carousel .img-thumbnail { padding-right: 1px; padding-left: 1px; }
		.product-images .img-thumbnail .inner { border: 1px solid <?php echo esc_html( $dark ? $color_dark_3 : '#e7e7e7' ); ?>; }
	<?php endif; ?>
	.product-thumbs-slider.owl-carousel .img-thumbnail { border-color: <?php echo esc_html( $dark ? $color_dark_3 : '#e7e7e7' ); ?>; }
<?php endif; ?>
...
<?php elseif ( 'side' == $b['mobile-panel-type'] ) : ?>
	...
	html.panel-opened {
		...
		margin-<?php echo porto_filter_output( $mobile_panel_pos ); ?>: 260px !important;
		margin-<?php echo 'left' == $mobile_panel_pos ? 'right' : 'left'; ?>: -260px !important;
	}
...
<?php if ( $b['mainmenu-toplevel-hbg-color'] ) : ?>
	...
	<?php else : ?>
		.header-side-nav .sidebar-menu > li.menu-item > a,
		.toggle-menu-wrap .sidebar-menu > li.menu-item > a { border-top-color: transparent; }
	<?php endif; ?>
<?php endif; ?>
<?php if ( $b['mainmenu-toplevel-link-color']['hover'] ) : ?>
	.header-side-nav .sidebar-menu > li.menu-item:hover > a,
...
<?php if ( class_exists( 'Woocommerce' ) && isset( $b['add-to-cart-font'] ) && ! empty( $b['add-to-cart-font']['font-family'] ) ) : ?>
	...
	.single-product .yith-wcwl-add-to-wishlist { font-family: <?php echo sanitize_text_field( $b['add-to-cart-font']['font-family'] ); ?>; }
<?php endif; ?>
...
<?php if ( class_exists( 'Woocommerce' ) ) : ?>
	...
	.product-categories li:not(.current) > a:hover { color: #7b858a !important; text-decoration: underline; }
	...
	<?php if ( isset( $b['product_variation_display_mode'] ) && 'button' == $b['product_variation_display_mode'] ) : ?>
		.single-product .variations .label label { line-height: 35px; }
	<?php endif; ?>
...
<?php if ( ! empty( $b['show-skeleton-screen'] ) ) : ?>
	...
	<?php if ( class_exists( 'Woocommerce' ) && ( in_array( 'product', $b['show-skeleton-screen'] ) || in_array( 'quickview', $b['show-skeleton-screen'] ) ) ) : ?>
		...
		/* quickview */
		.skeleton-body.product .summary-before { padding-top: 58%; }
		...
		.skeleton-body.product .summary-before:before {
			background-image: linear-gradient(<?php echo esc_html( $skeleton_color ); ?> 100%, transparent 0),
				...
				linear-gradient(<?php echo esc_html( $skeleton_color ); ?> 100%, transparent 0),
				linear-gradient(<?php echo esc_html( $skeleton_color ); ?> 100%, transparent 0);
			background-size: 100% 79%, 23% 19%, 23% 19%, 23% 19%, 23% 19%;
	...
	<?php if ( class_exists( 'Woocommerce' ) && in_array( 'shop', $b['show-skeleton-screen'] ) ) : ?>
		.products.skeleton-body li:before {
			content: ''; display: block; padding-top: calc(100% + 110px); background-repeat: no-repeat;
			background-image: linear-gradient(<?php echo esc_html( $skeleton_color ); ?> 100%, transparent 0),
			...
			background-position: <?php echo porto_filter_output( $left ); ?> 0, <?php echo porto_filter_output( $left ); ?> calc(62% + 20px), <?php echo porto_filter_output( $left ); ?> calc(62% + 44px), <?php echo porto_filter_output( $left ); ?> calc(62% + 66px), <?php echo porto_filter_output( $left ); ?> calc(62% + 92px);
		}
...
<?php if ( defined( 'ELEMENTOR_VERSION' ) ) : ?>
	.container-fluid .elementor-section { margin-left: -<?php echo (int) $b['grid-gutter-width']; ?>px; margin-right: -<?php echo (int) $b['grid-gutter-width']; ?>px; padding-left: <?php echo (int) $b['grid-gutter-width'] / 2; ?>px; padding-right: <?php echo (int) $b['grid-gutter-width'] / 2; ?>px; }
	.container .elementor-row { margin-left: -<?php echo (int) $b['grid-gutter-width'] / 2; ?>px; margin-right: -<?php echo (int) $b['grid-gutter-width'] / 2; ?>px; }
	.elementor-column-gap-default > .elementor-row > .elementor-column > .elementor-element-populated { padding: <?php echo (int) $b['grid-gutter-width'] / 2; ?>px }
<?php endif; ?>
