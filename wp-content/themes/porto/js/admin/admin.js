...
jQuery( function ( $ ) {
	...
	if ( $( '.blocks-wrapper .blocks-list' ).length ) {
		...
		var porto_blocks_cur_page = 1, porto_blocks_total_page = parseInt( $( '.blocks-wrapper .category-list a.active' ).data( 'total-page' ), 10 ), demo_filters = false, text_filter = '';
		$( '.blocks-wrapper .category-list a' ).on( 'click', function ( e, cur_page ) {
			e.preventDefault();
			...
			var $this = $( this ),
				...
				loaddata = { action: 'porto_studio_filter_category', category_id: cat, count_per_page: limit, wpnonce: porto_studio.wpnonce, page: porto_blocks_cur_page, post_id: porto_studio.post_id, text_filter: $( '.demo-filter #s' ).val() };
		...
		$( '.blocks-wrapper .demo-filter .filter1' ).on( 'change', function ( e ) {
			...
			$( '.blocks-wrapper .demo-filter .btn' ).trigger( 'click', [ false ] );
		} );
		$( '.blocks-wrapper .demo-filter .filter2' ).on( 'change', function ( e ) {
			$( '.blocks-wrapper .demo-filter .btn' ).trigger( 'click', [ false ] );
		} );
		$( '.blocks-wrapper .demo-filter .btn' ).on( 'click', function ( e, clickbtn ) {
			e.preventDefault();
			if ( false === clickbtn ) {
				var $this = $( this );
	...
	$( document.body ).on( 'click', '#porto-studio-editor-button, #porto-elementor-panel-porto-studio, #vce-porto-studio-trigger, #gutenberg-porto-studio-trigger', function ( e ) {
		e.preventDefault();
		var $this = $( this );
	...
	function porto_import_finished( options ) {
		...
		setTimeout( function () {
			...
			jQuery( '.porto-install-demo .porto-demo-install' ).css( 'padding-left', 0 );
			jQuery( '.porto-remove-demo .btn' ).prop( 'disabled', false );
			removeAlertLeavePage();
		}, 3000 );
	}
	...
	jQuery( document ).on( 'change', '.pagebuilder-selector input[type="radio"]', function () {
		...
		if ( 'vc' == $( this ).val() && $o.children( '.porto-vc-addon' ).length ) {
	...
	jQuery( document ).ready( function ( $ ) {
		...
		if ( document.body.classList.contains( 'block-editor-page' ) ) {
			...
			// add studio button
			setTimeout( function () {
				$( '#editor .edit-post-header-toolbar__left' ).append( '<span id="gutenberg-porto-studio-trigger" class="vcv-ui-navbar-control" title="Porto Studio"><i class="porto-icon-studio"></i></span>' );
			}, 100 );
		}
		...
		var porto_fn_remove_demo = function ( options, all_checked ) {
			...
			if ( option !== undefined ) {
				$( '.porto-remove-demo .btn' ).prop( 'disabled', true );
				...
				var postdata = { action: 'porto_sw_remove_demo', wpnonce: porto_setup_wizard_params.wpnonce };
				...
				$.ajax( {
					...
					failure: function () {
						$( '.porto-remove-demo .btn' ).prop( 'disabled', false );
		...
		$( '.porto-remove-demo .btn' ).on( 'click', function ( e ) {
			...
			$( this ).closest( '.porto-remove-demo' ).find( 'input[type="checkbox"]:checked' ).each( function () {
				var val = $( this ).val();
				if ( val ) {
